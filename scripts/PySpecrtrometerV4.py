...
load = Image.open(r'/home/pi/Desktop/spectrum(1).png')
...
class App:
	...
	def __init__(self, args, window, window_title, video_source=0):
		...
		def snapshot():
			...
			if ret:
				...
				picturePath = "/home/pi/Desktop/Pictures"
				graphPath = "/home/pi/Desktop/Graphs"
				cv2.imwrite(os.path.join(picturePath,"spectrum-" + now + ".jpg"),
				            cv2.cvtColor(graphdata[0], cv2.COLOR_RGB2BGR))
				#print(graphdata[1]) #wavelengths
				#print(graphdata[2]) #intensities
				f = open(os.path.join(graphPath,now+'.csv'), 'w')
		...
		self.decoration.grid(row=0, column=2, columnspan=3, padx=10, pady=0)

		#control panel items:
		# load the  image file
		self.decorate = ImageTk.PhotoImage(load)
		self.decoration.create_image(2, 2, image=self.decorate, anchor=tkinter.NW)
		#calibrate Red button
		self.calbutton1 = tkinter.Button(self.control_frame, text="Calibrate Red", width=15,height=2,
                                  fg="yellow", bg="red", activebackground='red', command=lambda: self.calibrateRed(self.data()))
		self.calbutton1.grid(row=1, column=2,padx=10,pady=10)
		#calibrate Green button
		self.calbutton2 = tkinter.Button(self.control_frame, text="Calibrate Green", width=15, height=2,
                                  fg="yellow", bg="green", activebackground='green', command=lambda: self.calibrateGreen(self.data(),maxRed))
		self.calbutton2.grid(row=1, column=3,padx=10, pady=10)
		...
		#Sample label
		self.lbsample = tkinter.Label(self.bottom_frame, text="Sample Row:")
		self.lbsample.grid(row=1, column=6, pady=20, sticky="n")
		...
		self.peakholdbtn.grid(row=3, column=4, columnspan=2, padx=0, pady=0)
	...
	def calibrateRed(self,graphdata):
		...
		self.calbutton1.configure(text="Calibrated Red", fg="black",
	...
	def calibrateGreen(self, graphdata, maxRed):
		...
		self.vid.recalibrate(calibration)
		self.calbutton2.configure(text="Calibrated Green", fg="black",
                           bg="yellow", activebackground='yellow')
